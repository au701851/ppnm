...
public readonly int size1, size2; //size1 is number of rows, size2 is number of coloumns
public double[][] data;
...
public static matrix operator+ (matrix a, matrix b){
	...
	for(int j=0;j<a.size2;j++)
		for(int i=0;i<a.size1;i++) c[i,j]=a[i,j]+b[i,j];
		return c;
}
...
public static matrix operator-(matrix a){
	matrix c = new matrix(a.size1,a.size2);
	for(int j=0;j<a.size2;j++)
	for(int i=0;i<a.size1;i++) c[i,j]=-a[i,j];
	return c;
}
...
public static matrix operator/(matrix a, double x){
	matrix c=new matrix(a.size1,a.size2);
	for(int j=0;j<a.size2;j++)
	for(int i=0;i<a.size1;i++) c[i,j]=a[i,j]/x;
	return c;
}
...
public bool approx(matrix B,double acc=1e-6, double eps=1e-6){
	...
	for(int i=0;i<size1;i++) for(int j=0;j<size2;j++)
		if(!approx(this[i,j],B[i,j],acc,eps)) return false;
	return true;
}
