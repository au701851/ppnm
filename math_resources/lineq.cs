...
public class lineq{
	...
	public static void QRGSdecomp(ref matrix A, ref matrix R){
		...
		//Iterating over the coloumns of A
			R[i,i] = A[i].norm(); //The diagonal elements of R are the norms of the vectors of A
			A[i]/=R[i,i]; //The first vector of A is normalized
			for(int j = i+1; j<A.size2; j++){
				//Iterating over the rest of the row
				R[i,j] = A[i].dot(A[j]); //Projecting coloumn
				A[j] -= A[i]*R[i,j]; 
				}
			}
	}
